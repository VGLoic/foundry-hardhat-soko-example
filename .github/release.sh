...
npm run hardhat soko push -- --artifact-path ./out --tag "assured-counter:v$RELEASE_TAG_WITHOUT_PREFIX" && echo "Successfully pushed release artifact" || echo "Failed to push release, we assume here that this is because the release already exists. Still room for improvment here."
